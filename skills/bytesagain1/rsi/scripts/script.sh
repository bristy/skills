#!/usr/bin/env bash
# rsi — Rsi reference tool. Use when working with rsi in finance contexts.
...
show_help() {
    cat << 'HELPEOF'
rsi v$VERSION — Rsi Reference Tool
...
## What is Rsi?
Rsi (rsi) is a specialized tool/concept in the finance domain.
It provides essential capabilities for professionals working with rsi.

## Key Concepts
- Core rsi principles and fundamentals
- How rsi fits into the broader finance ecosystem  
- Essential terminology every practitioner should know

## Why Rsi Matters
Understanding rsi is critical for:
...
## Getting Started
1. Understand the basic rsi concepts
...
cmd_instruments() {
    cat << 'EOF'
# Rsi — Instruments & Tools Overview

## Primary Instruments
- Core tools used in rsi operations
...
cmd_glossary() {
    cat << 'EOF'
# Rsi — Key Terms & Definitions

## Core Terminology
- **Rsi**: The primary subject of this reference
...
case "$CMD" in
    ...
    version|--version|-v) echo "rsi v$VERSION — Powered by BytesAgain" ;;
    *) echo "Unknown: $CMD"; echo "Run: rsi help"; exit 1 ;;
esac
