#!/usr/bin/env python3
"""
MiniMax Image Understanding via MCP
...
def understand_image(image_path, prompt):
    """Analyze image using MiniMax MCP."""
    ...
    requests = [
        ...
        {'jsonrpc': '2.0', 'id': 2, 'method': 'tools/call',
         'params': {'name': 'understand_image',
                    'arguments': {'image_source': image_path, 'prompt': prompt}}}
    ]
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: understand_image.py <image_path> <prompt>", file=sys.stderr)
        sys.exit(1)
    understand_image(sys.argv[1], sys.argv[2])
